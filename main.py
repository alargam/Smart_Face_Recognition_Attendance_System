...
# Final Professional Version - Dynamic Paths
# ==============================================================================
...
# This line gets the absolute path of the folder where main.py is located
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

# Define absolute paths for all directories
PATH_TRAINING_IMG = os.path.join(BASE_DIR, "TrainingImage")
PATH_TRAINING_LABEL = os.path.join(BASE_DIR, "TrainingImageLabel")
PATH_STUDENT_DETAILS = os.path.join(BASE_DIR, "StudentDetails")
PATH_ATTENDANCE = os.path.join(BASE_DIR, "Attendance")
PATH_HAARCASCADE = os.path.join(BASE_DIR, "haarcascade_frontalface_default.xml")
...
def contact():
    mess._show(title='Contact us', message="Please contact us on : 'support@gmail.com' ")

def check_haarcascadefile():
    if os.path.isfile(PATH_HAARCASCADE):
        pass
    else:
        mess._show(title='Missing File', message='haarcascade_frontalface_default.xml is missing.\nPlease verify it exists in: ' + BASE_DIR)
        window.destroy()

def save_pass():
    assure_path_exists(PATH_TRAINING_LABEL)
    pass_path = os.path.join(PATH_TRAINING_LABEL, "psd.txt")
...
def TakeImages():
    ...
    assure_path_exists(PATH_STUDENT_DETAILS)
    assure_path_exists(PATH_TRAINING_IMG)
    
    serial = 0
    csv_path = os.path.join(PATH_STUDENT_DETAILS, "StudentDetails.csv")
    ...
    if ((name.isalpha()) or (' ' in name)):
        ...
        while (True):
            ...
            for (x, y, w, h) in faces:
                ...
                # Save Image with dynamic path
                img_name = f"{name}.{serial}.{Id}.{sampleNum}.jpg"
                cv2.imwrite(os.path.join(PATH_TRAINING_IMG, img_name), gray[y:y + h, x:x + w])
...
def TrainImages():
    ...
    faces, ID = getImagesAndLabels(PATH_TRAINING_IMG)
    try:
    ...
    recognizer.save(os.path.join(PATH_TRAINING_LABEL, "Trainner.yml"))
...
def TrackImages():
    check_haarcascadefile()
    assure_path_exists(PATH_ATTENDANCE)
    assure_path_exists(PATH_STUDENT_DETAILS)
    ...
    recognizer = cv2.face.LBPHFaceRecognizer_create()
    trainer_path = os.path.join(PATH_TRAINING_LABEL, "Trainner.yml")
    ...
    try:
        ts = time.time()
        date = datetime.datetime.fromtimestamp(ts).strftime('%d-%m-%Y')
        attendance_file = os.path.join(PATH_ATTENDANCE, "Attendance_" + date + ".csv")
...
window.title("Attendance System - Delivero Tech")
...
message3 = tk.Label(window, text="Smart Attendance System (Face Recog)" ,fg="white",bg="#262523" ,width=55 ,height=1,font=('times', 29, ' bold '))
...
# Initial Count Check
csv_path_check = os.path.join(PATH_STUDENT_DETAILS, "StudentDetails.csv")
res = 0
if os.path.isfile(csv_path_check):
    with open(csv_path_check, 'r') as csvFile1:
        reader1 = csv.reader(csvFile1)
        for l in reader1:
            res = res + 1
    res = (res // 2) - 1
